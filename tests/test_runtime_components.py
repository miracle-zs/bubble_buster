...
""",
    ...
    assert account_runtimes["acc01"]["morning_protection_hour"] == 7
    assert account_runtimes["acc01"]["morning_protection_minute"] == 55
    assert account_runtimes["acc01"]["morning_protection_min_hold_hours"] == 6.0
...
""",
    ...
    def test_create_components_exposes_account_cooling_off_retry_override(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            cfg_path = tmp_path / "config.ini"
            db_path = tmp_path / "state.db"
...
""",
            ...
            self.assertEqual(account_runtimes["acc01"]["cooling_off_retry_count"], 0)
            self.assertEqual(account_runtimes["acc01"]["cooling_off_retry_delay_sec"], 0)
            self.assertEqual(account_runtimes["acc02"]["cooling_off_retry_count"], 1)
            self.assertEqual(account_runtimes["acc02"]["cooling_off_retry_delay_sec"], 30)
            self.assertEqual(account_runtimes["acc02"]["strategy"].cooling_off_retry_count, 1)
            self.assertEqual(account_runtimes["acc02"]["strategy"].cooling_off_retry_delay_sec, 30)
