"""Read-only architecture audit: temporary SQLite and mocked network only.

Run from the repository root: python docs/audits/2026-09-22/probe.py
Assertions describe correct behavior; failures reproduce audit findings.
"""
import sys
import tempfile
import unittest
...
class AuditProbe(unittest.TestCase):
    def setUp(self):
        ...
        self.store = StateStore(str(Path(self.tmp.name) / 'state.db'),
                                str(ROOT / 'schema.sql'), account_id='audit')
        self.store.init_schema()

    def test_old_order_event_must_not_resurrect_filled_order(self):
        order = dict(symbol='TESTUSDT', orderId=123, type='LIMIT', side='BUY')
        self.store.upsert_exchange_order_state(
            dict(order, status='FILLED', executedQty='1'), source='ORDER_TRADE_UPDATE',
            event_time_utc='2026-09-22T10:00:02+00:00')
        self.store.upsert_exchange_order_state(
            dict(order, status='NEW', executedQty='0'), source='ORDER_TRADE_UPDATE',
    ...
    def test_stream_update_during_rest_must_survive_in_database(self):
        entered, release, applied = Event(), Event(), Event()
        errors = []
        client = MagicMock()
        payload = dict(assets=[dict(asset='USDT', walletBalance='100',
                       unrealizedProfit='0', marginBalance='100')], positions=[])
        ...
        def stream():
            try:
                provider.apply_stream_update(balances=[dict(a='USDT', wb='200')],
                    positions=[], captured_at_utc='2026-09-22T10:01:01+00:00')
            except Exception as exc:
                errors.append(exc)
        ...
        self.assertEqual(self.store.get_latest_account_state()['wallet_balance'], 200)

    def test_stale_rest_open_orders_must_not_resurrect_filled_order(self):
        order = dict(symbol='TESTUSDT', orderId=123, type='LIMIT', side='BUY')
        self.store.upsert_exchange_order_state(
            dict(order, status='FILLED', executedQty='1'), source='ORDER_TRADE_UPDATE',
            event_time_utc='2026-09-22T10:00:02+00:00')
        # REST began before the fill; its response arrives after the stream event.
        self.store.reconcile_open_order_state([dict(order, status='NEW', executedQty='0')])
        row = self.store.get_exchange_order_state(symbol='TESTUSDT', order_id=123)
        self.assertEqual(row['status'], 'FILLED')

    def test_cashflow_wait_must_not_block_protection_dispatch(self):
        ...
        cfg = ServiceRuntimeConfig('UTC', 7, 40, 120, True, True,
                                   11, 55, 60, 3, 1, False,
                                   account_task_timeout_sec=0.1)
        service = StrategyRuntimeService(MagicMock(), MagicMock(), cfg,
                  account_runtimes={'audit': dict(mode='full', balance_sampler=sampler)})
        for method in ['_run_entry_if_due', '_run_noon_protection_if_due',
                       '_run_morning_protection_if_due', '_run_hourly_exchange_take_profit_if_due',
                       '_run_orphan_exit_order_cleanup_if_due', '_run_manage_if_due',
        ...
        worker = Thread(target=service.run_cycle, kwargs={
            'now_local': datetime(2026, 9, 22, 11, 55, 5, tzinfo=timezone.utc)})
        worker.start()
        try:
            self.assertTrue(entered.wait(2))
