        ...
        with self._connect_ctx() as conn:
            for record in records or []:
                ...
                if event_time_ms <= 0 or not income_type:
                    continue
                if tran_id:
                    # Binance can reuse a tranId for the realized PnL and the
                    # commission generated by the same fill.  The income type
                    # is therefore part of the identity; tranId alone would
                    # silently drop one of those rows.
                    unique_source = f"tran:{tran_id}:{income_type}"
                elif trade_id:
                ...
                if tran_id:
                    existing = conn.execute(
                        ...
                        WHERE account_id = ? AND tran_id = ? AND income_type = ?
                        LIMIT 1
                        """,
                        (self.account_id, tran_id, income_type),
                    ).fetchone()
                    if existing is not None:
                        continue
                cursor = conn.execute(
